 ...
 *
 * Projeto alvo: gameprice-bd3ba (credenciais via conta de serviço no .env).
 *
    ...
    try {
      ...
      throw new Error(
        `Arquivo de credenciais do Firebase não encontrado:\n  ${abs}\n\n` +
          `Corrija FIREBASE_SERVICE_ACCOUNT_PATH no .env para o JSON do projeto ${EXPECTED_PROJECT_ID} ` +
          `(Console Firebase → Contas de serviço → Gerar nova chave privada). ` +
          `Dica: coloque em ./secrets/ e use FIREBASE_SERVICE_ACCOUNT_PATH=./secrets/seu-arquivo.json`
      );
    }
    let json;
    try {
    ...
    if (json.project_id && json.project_id !== EXPECTED_PROJECT_ID) {
      console.error(
        `Aviso: project_id no JSON é "${json.project_id}", esperado "${EXPECTED_PROJECT_ID}". ` +
          "Confirme que baixou a chave do projeto GamePrice correto."
      );
    }
    if (getApps().length === 0) {
...
async function main() {
  ...
  if (prune) {
    ...
    console.error(
      "Dica: se o Firestore tiver documentos antigos, rode com --prune ou FIRESTORE_PRUNE_ORPHANS=1."
    );
  }
...
O que fazer:
1. Firebase Console (projeto ${EXPECTED_PROJECT_ID}) → Contas de serviço → "Gerar nova chave privada"
2. Substitua o arquivo em FIREBASE_SERVICE_ACCOUNT_PATH no .env
3. Confirme que o project_id no JSON é ${EXPECTED_PROJECT_ID}
4. Ative o Firestore no modo desejado (teste ou produção) no mesmo projeto
